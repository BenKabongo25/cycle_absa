...
class AnnotationsTextFormerBase(object):
    ...
    @classmethod
    def get_annotations_text_former(cls, args):
        if args.annotations_text_type is AnnotationsTextFormerType.PARAPHRASE:
            annotations_text_former = ParaphraseAnnotationsTextFormer(args)
        elif args.annotations_text_type is AnnotationsTextFormerType.GAS_ANNOTATION_STYLE:
            annotations_text_former = GASAnnotationStyleAnnotationsTextFormer(args)
        else: # GAS_EXTRACTION_STYLE
            annotations_text_former = GASExtractionStyleAnnotationsTextFormer(args)
...
class ParaphraseAnnotationsTextFormer(AnnotationsTextFormerBase):
    ...
    sentiment_paraphrases_vk = dict(
        list(map(lambda kv: (kv[1],kv[0]), sentiment_paraphrases_kv.items()))
    )
    ...
    def polarity_annotation_to_paraphrase(self, p: str) -> str:
        return self.sentiment_paraphrases_kv.get(p, "")

    def polarity_paraphrase_to_annotation(self, p: str) -> str:
        return self.sentiment_paraphrases_vk.get(p, "")
...
if __name__ == "__main__":
    ...
    def test_paraphrase():
        ...
        former = GASExtractionStyleAnnotationsTextFormer(args)

        annotations = [("open sesame", "restaurant general", "positive", "come back")]
        text = former.multiple_annotations_to_text(annotations)
        print("Text:", text)
        print("Ref Annotations:", annotations)

        annotations = former.multiple_text_to_annotations(text)
